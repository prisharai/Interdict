...
correctable for database agents.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{fig1_literal_rule_satisfaction.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig3_closed_loop.pdf}
...
agent issues and decides whether it may run. One fact about SQL suffices for what
follows. The tasks operate on a \emph{table}---a grid of
...
Every rate below is computed automatically from the logged transcripts
(Sec.~\ref{sec:coding}), reported with a Wilson 95\% confidence interval, and
compared against the opaque-error baseline with Fisher's exact test.

Two figures carry the results, with all cell values and tests collected in
Table~\ref{tab:results}. Figure~\ref{fig:headline} shows literal rule
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{fig2_recovery.pdf}
...
of SQL (57--95\% of trials had at least one such turn; Figure~\ref{fig:protocol}a),
which is a real caveat on
...
pattern is unchanged (Figure~\ref{fig:protocol}b): pooled literal rule satisfaction
rises from 83\% under the opaque baseline to 97--100\% under the richer conditions.
Per-condition
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig4_protocol_failure.pdf}
  \caption{\textbf{The gaming effect survives the protocol-failure caveat.}
    \textbf{(a)} Share of trials with at least one non-SQL (prose) turn, by model
    and denial richness. Claude Haiku 4.5 (blue) returns prose in 57--95\% of
    trials; GPT-5.5 (red) almost never does (0--3\%), so its rates rest on genuine
    SQL attempts. \textbf{(b)} Sensitivity analysis: with every protocol-failure
    trial excluded, pooled literal rule satisfaction still rises from 83\% under
    the opaque baseline to 97--100\% under the richer denial conditions---the
    effect is not an artifact of the prose turns.}
  \label{fig:protocol}
\end{figure}
